...
def get_emb(
    ...
    model.eval()
    x, y = eval_dataset(model, dl_eval, skip_head)  # the eval of cub dataset: torch.Size([5924, 128]) torch.Size([5924]) False
    y = y.cuda()
    if world_size > 1:
        print('*********************')
...
def eval_dataset(model, dl, skip_head):
    ...
    for x, y in dl:  # torch.Size([100, 3, 224, 224]) label y: torch.Size([100]) -> 100*i = all sample numbers of (100-200) classes in cub dataset
        with torch.no_grad():
            ...
            z = model(x, skip_head=skip_head)
            all_x.append(z[:,:384])  # the outputs of model: torch.Size([100, 128])
        all_y.append(y)
